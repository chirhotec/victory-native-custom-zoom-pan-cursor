...
getPinchDistance = nativeEvent => {
  return Math.abs(
    nativeEvent.touches[0].locationX - nativeEvent.touches[1].locationX
  );
};
...
getPinchPosition = nativeEvent => {
  return (
    (nativeEvent.touches[0].locationX + nativeEvent.touches[1].locationX) / 2
  );
};
  ...
  return [
    {
      ...
      eventHandlers: {
        ...
        onTouchStart: (evt, targetProps, eventKey, ctx) => {
          ...
          let zoomMutators = NativeZoomHelpers.onTouchStart(
            evt,
            targetProps,
            eventKey,
            ctx
          ...
          switch (targetProps.gestureState) {
            case GESTURE_STATE.READY:
              if (evt.nativeEvent.touches.length > 1) {
                ...
                let distanceMoved = Math.abs(
                  targetProps.startX - evt.nativeEvent.locationX
                );
                if (distanceMoved >= targetProps.cursorMovementThreshold) {
                  mutators.push(setGestureStateCursor);
                  let cursorMutators = CursorHelpers.onMouseMove(
                    evt,
                    targetProps
                  );
        ...
        onTouchPinch: (evt, targetProps, eventKey, ctx) => {
          ...
          switch (targetProps.gestureState) {
            ...
            case GESTURE_STATE.PAN_OR_ZOOM:
              ...
              if (
                Math.abs(pinchDistance - targetProps.pinchDistance0) >
                targetProps.zoomOnPinchThreshold
              ) {
                mutators.push(setGestureStateZoom);
                zoomMutators = NativeZoomHelpers.onTouchPinch(
                  evt,
                  targetProps,
                  eventKey,
                  ctx
                ...
                Math.abs(pinchPosition - targetProps.pinchPosition0) >
                targetProps.panOnPinchThreshold
              ) {
                mutators.push(setGestureStatePan);
                zoomMutators = NativeZoomHelpers.onTouchMove(
                  evt,
                  targetProps,
                  eventKey,
                  ctx
...
export default class TimelineControlsContainer extends VictoryContainer {
  ...
  static defaultEvents = printEvents;
}
